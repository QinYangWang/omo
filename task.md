...
- [x] E0-003：Spike：通过 Extension command bridge 将外部 Prompt 与 Abort 注入同一个原生 AgentSession（`5482180`）
- [x] E0-004：Spike：`omo` 使用项目锁定的 Pi 依赖启动原生 TUI并显式加载 Extension，无需全局安装 Pi（`9909fd3`）
- [x] E0-GATE：真实 Pi TUI Prompt 可在测试客户端 token 级显示，外部 Prompt 可由同一 runtime 执行，退出后无残留资源
...
- [x] E1-004：将 native sequence 去重后映射为现有 Host event sequence，并抑制 attachment 活跃时的文件 watcher 重复事件（`5b6ae31`）
- [x] E1-GATE：验证 attach 竞争、stale generation、heartbeat 超时、daemon 重启与单 Session 单执行者
...
- [x] E3-003：实现原生 TUI 启动失败、daemon 不可达与版本不兼容的可操作错误和安全回退（`cd5ddee`）
- [x] E3-GATE：验证本机无需全局 Pi、原生 TUI 功能无降级、远程 Host 路径无回归

### E4：Web/Desktop 双向控制

- [x] E4-001：在现有 Session API 中暴露安全的 execution owner/attachment 状态，不泄漏 process credential
- [x] E4-002：通过 broker 将 Web Prompt/Abort 路由到 native owner，禁止 selected native 失败时隐式创建 headless runtime
- [x] E4-003：验证 native event 的 SQLite replay、客户端 SSE 重连与 Extension 重连去重
- [x] E4-004：保留无 Extension 外部 Pi 的 JSONL 校准路径，并验证不会与 native event 双重渲染
- [x] E4-GATE：完成 Pi native TUI ↔ daemon ↔ Web/Desktop 双向实时 E2E，一个客户端断线不影响执行

### E5：故障恢复与默认切换

- [x] E5-001：实现 idle 边界上的显式 native detach → headless resume；运行中崩溃只标记 interrupted，不自动重复 dispatch
- [x] E5-002：在所有 Gate 通过后将本机 `omo` 默认切换为 Pi 原生 TUI，保留一个发布周期的 legacy fallback
- [x] E5-003：完成 Pi package 安装/升级、版本兼容、运维与故障排查文档
- [x] E5-GATE：通过单执行者竞争、Pi/daemon 异常退出、重启重连、事件去重和真实浏览器验收矩阵
