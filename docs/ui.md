...
- “导入会话”与项目行“新会话”按钮只在 hover 或键盘聚焦时显示；PROJECTS 行的添加按钮常显但保持低对比。“导入会话”只刷新并展示磁盘上已有的 Pi Session，不复制 JSONL、不创建 fork，也不改写会话树。
...
- 置顶会话固定在项目内最前，按创建时间从新到旧排序；归档会话从侧边栏隐藏，可在设置「已归档」恢复或经二次确认后永久删除 Session JSONL。正在运行或由原生 Pi TUI attach 的会话不允许删除。
