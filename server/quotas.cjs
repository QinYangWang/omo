...
function loadQuotas() {
  ...
  const source = path.join(path.dirname(packageFile), "src", "lib", "quotas.ts");
...
async function fetchQuotas(piService, agentDir, force = false) {
  ...
  let stored = {};
  try { stored = JSON.parse(fs.readFileSync(path.join(agentDir, "auth.json"), "utf8")); } catch {}
  ...
  return {
    ...
    items: results.map(({ provider, result }) => ({
      ...
      windows: result.success
        ? result.data.windows.map((window) => ({ ...window, resetsAt: new Date(window.resetsAt).toISOString() }))
        : [],
    })),
  };
}
